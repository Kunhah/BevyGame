...
pub struct Buff {
    ...
    pub duration: u32, // every unit of duration will be equivalent of 60/16 seconds, that means everytime a the function to get the turn order is called, 60 >> 4 seconds passes and this will be decremented by 1
...
pub trait CharacterBase: Send + Sync + Debug {
    ...
    fn damage(&mut self, mut amount: u32, damage_type: DamageType);
    fn buff(&mut self, stat: Stat, multiplier: f32, duration: u8);
    fn debuff(&mut self, stat: Stat, multiplier: f32, duration: u8);
...
impl CharacterBase for Petrus {
    ...
    fn damage(&mut self, mut amount: u32, damage_type: DamageType) {
        ...
        let reduced = (amount as  u32 * 85) as u32;
...
impl CharacterBase for Rina {
    ...
    fn damage(&mut self, mut amount: u32, damage_type: DamageType) {
        ...
        let reduced = (amount as  u32 * (1.0 - (self.base_character.agility as  u32 / 100.0))) as u32;
        self.base_character.health = self.base_character.health.saturating_sub(reduced);
    }
...
impl TurnManager {
    pub fn new(characters: Vec<Box<dyn CharacterBase>>) -> Self {
        ...
        let turn_threshold = avg_agility;

        Self { characters, turn_threshold }
    }

    pub fn calculate_turn_order(&mut self) -> Vec<u32> {
        ...
        for character in self.base_character.characters.iter_mut() {
            let mut acc = character.accumulated_agility() + character.agility() as  u32;
            while acc >= self.base_character.turn_threshold {
                acc -= self.base_character.turn_threshold;
...
pub enum AbilityEffect {
    Damage(u32, u32, DamageType),
    Heal(u32, u32),
    Buff(String, f32),   // e.g. "agility", 1.2 multiplier
    Debuff(String, f32),
...
impl Ability {
    pub fn get_level(&self) -> u8 { self.id << 8 }
    pub fn get_sub_id(&self) -> u8 { self.id >> 8 }
}
...
pub struct AbilityNode {
    pub ability: Ability,
    pub left: Option<Rc<RefCell<AbilityNode>>>,
    pub right: Option<Rc<RefCell<AbilityNode>>>,
}

impl AbilityNode {
    pub fn new(ability: Ability) -> Rc<RefCell<Self>> {
        Rc::new(RefCell::new(AbilityNode {
            ability,
            left: None,
            right: None,
        }))
...
pub struct AbilityTree {
    pub root: Option<Rc<RefCell<AbilityNode>>>,
}
...
impl AbilityTree {
    ...
    fn insert_node(current: Rc<RefCell<AbilityNode>>, new_node: Rc<RefCell<AbilityNode>>) {
        ...
        let new_id = new_node.borrow().ability.id;
        let current_id = current.borrow().ability.id;
        ...
        match new_id.cmp(&current_id) {
            ...
            Ordering::Equal => {
                ...
                current.borrow_mut().ability = new_node.borrow().ability.clone();
            }
        }
    }
    ...
    fn find_node(node: Option<Rc<RefCell<AbilityNode>>>, id: u16) -> Option<Ability> {
        if let Some(n) = node {
    ...
    fn find_all_level(level: u8) -> Option<Vec<Ability>> {
    ...
    fn collect_level_abilities(
        node: Option<Rc<RefCell<AbilityNode>>>,
        level: u8,
    ...
    fn collect_all(node: Option<Rc<RefCell<AbilityNode>>>, all: &mut Vec<Ability>) {
        if let Some(n) = node {
            let n_borrow = n.borrow();
...
pub fn handle_ability(ability: &Ability, affected_characters: &Vec<Box<dyn CharacterBase>> ) -> Vec<Box<dyn CharacterBase>> {
        ...
        for effect in effects {
            match effect {
                AbilityEffect::Heal { floor, ceiling } => {
                    let amount = rand::thread_rng().gen_range(floor..ceiling);
                ...
                AbilityEffect::Buff { stat, multiplier } => {
                    character.buff(stat, multiplier, ability.duration);
                }
